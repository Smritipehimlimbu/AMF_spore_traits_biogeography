...
library(pacman)
pacman::p_load(readr, dplyr, tidyverse, factoextra, MASS, mgcv, ggplot2, patchwork, ggpubr, MASS, bestNormalize)
```
#data upload
```{r}
...
###############convex hull for traits
colnames(df_pca.cwm.scores.traits)

scores_long <- df_pca.cwm.scores.traits %>%
  pivot_longer(cols = c(Volume, Ornamentation, Shape, Investment, Color),
               names_to = "trait", values_to = "value")


# NEW: Filter top 25% values per trait
scores_top_trait <- scores_long %>%
  group_by(trait) %>%
  filter(value >= quantile(value, 0.75, na.rm = TRUE))  # Keep top 25%

# Calculate convex hulls for each trait using filtered data
hull_data <- scores_top_trait %>%
  group_by(trait) %>%
  slice(chull(Comp.1, Comp.2))
ggplot(scores_long, aes(x = Comp.1, y = Comp.2)) +
  geom_hline(yintercept = 0, linetype = "dashed", color = "black") +
  geom_vline(xintercept = 0, linetype = "dashed", color = "black") +
  geom_point(alpha = 0.2, color = "gray60", size = 0.5) +
  geom_polygon(data = hull_data,
               aes(x = Comp.1, y = Comp.2, fill = trait, group = trait),
               alpha = 0.5, color = "black") +
  facet_wrap(~trait, scales = "fixed") +
  theme_minimal() +
  labs(x = "PC1", y = "PC2", title = "Convex Hulls of Top 25% Trait Values in PCA Space") +
  theme(
...
###############testing pc3  and their directionality
fig_pc3_prec.max <- ggplot(df_pca, aes(x = PC3, y = Prec.max.worldclim)) +
  geom_point(alpha = 0.6) +
  geom_smooth(method = "lm", se = TRUE, color = "blue") +
 stat_cor(method = "pearson",
...
fig_pc3_prec.seasonality <- ggplot(df_pca, aes(x = PC3, y = Prec.seasonality.worldclim)) +
  geom_point(alpha = 0.6) +
  geom_smooth(method = "lm", se = TRUE, color = "blue") +
 stat_cor(method = "pearson",
         aes(label = paste(..r.label.., ..rr.label..,..p.label.., sep = "~`,`~")),
...
fig_pc3_prec_annual <- ggplot(df_pca, aes(x = PC3, y = Prec.annual.worldclim)) +
  geom_point(alpha = 0.6) +
  geom_smooth(method = "lm", se = TRUE, color = "blue") +
 stat_cor(method = "pearson",
         aes(label = paste(..r.label.., ..rr.label..,..p.label.., sep = "~`,`~")),
...
fig_pc3_map <- ggplot(df_pca, aes(x = PC3, y = MAP)) +
  geom_point(alpha = 0.6) +
  geom_smooth(method = "lm", se = TRUE, color = "blue") +
 stat_cor(method = "pearson",
         aes(label = paste(..r.label.., ..rr.label..,..p.label.., sep = "~`,`~")),
...
fig_pc3_temp_seasonality <- ggplot(df_pca, aes(x = PC3, y = Temp.seasonality.worldclim)) +
  geom_point(alpha = 0.6) +
  geom_smooth(method = "lm", se = TRUE, color = "blue") +
 stat_cor(method = "pearson",
         aes(label = paste(..r.label.., ..rr.label..,..p.label.., sep = "~`,`~")),
...
fig_pc3_temp_min <- ggplot(df_pca, aes(x = PC3, y = Temp.min.worldclim)) +
  geom_point(alpha = 0.6) +
  geom_smooth(method = "lm", se = TRUE, color = "blue") +
 stat_cor(method = "pearson",
         aes(label = paste(..r.label.., ..rr.label..,..p.label.., sep = "~`,`~")),
...
#combine figures (PC3)
ggarranged_plot_PC3 <- ggarrange(fig_pc3_prec.max,fig_pc3_temp_seasonality,  fig_pc3_temp_min,fig_pc3_prec.seasonality, fig_pc3_prec_annual, fig_pc3_map,
  labels = c("a)", "b)", "c)", "d)","e)", "f)",
  ncol = 3, nrow = 2,             
  align = "hv" ,  font.label = list(size = 18, face = "bold")                 
))

ggarranged_plot_PC3_titled <- annotate_figure(
  ggarranged_plot_PC3,
  top = text_grob("Relationships Between PC3 and Environmental Variables", 
                  face = "bold", size = 20)
)
ggsave("PC3_relationships.tiff", plot = ggarranged_plot_PC3_titled, width = 14, height = 12, dpi = 300)

#################testing pc4 and their directionality
fig_pc4_pH <- ggplot(df_pca, aes(x = PC4, y = pH)) +
  geom_point(alpha = 0.6) +
  geom_smooth(method = "lm", se = TRUE, color = "blue") +
 stat_cor(method = "pearson",
         aes(label = paste(..r.label.., ..rr.label.., sep = "~`,`~")),
...
fig_pc4_prec.seasonality <- ggplot(df_pca, aes(x = PC4, y = Prec.seasonality.worldclim)) +
  geom_point(alpha = 0.6) +
  geom_smooth(method = "lm", se = TRUE, color = "blue") +
 stat_cor(method = "pearson",
         aes(label = paste(..r.label.., ..rr.label.., sep = "~`,`~")),
...
fig_pc4_prec_min <- ggplot(df_pca, aes(x = PC4, y = Prec.min.worldclim)) +
  geom_point(alpha = 0.6) +
  geom_smooth(method = "lm", se = TRUE, color = "blue") +
 stat_cor(method = "pearson",
         aes(label = paste(..r.label.., ..rr.label.., sep = "~`,`~")),
...
fig_pc4_wind <- ggplot(df_pca, aes(x = PC4, y = mean_annual_wind)) +
  geom_point(alpha = 0.6) +
  geom_smooth(method = "lm", se = TRUE, color = "blue") +
 stat_cor(method = "pearson",
         aes(label = paste(..r.label.., ..rr.label..,..p.label.., sep = "~`,`~")),
  ...
  theme_minimal() +
  labs(x = "PC4 Scores",
       y = "mean annual wind (m/s)")+
  theme_minimal(base_size = 16)

#combine figures (PC4)
ggarranged_plot_PC4 <- ggarrange(fig_pc4_pH, fig_pc4_prec_min, fig_pc4_prec.seasonality, fig_pc4_wind,
  labels = c("a)", "b)", "c)", "d)",  
  ncol = 4, nrow = 1,             
  align = "hv"  ,  font.label = list(size = 18, face = "bold")                 
))

ggarranged_plot_PC4_titled <- annotate_figure(
  ggarranged_plot_PC4,
  top = text_grob("Relationships Between PC4 and Environmental Variables", 
                  face = "bold", size = 20)
)
ggsave("PC4_relationships.tiff", plot = ggarranged_plot_PC4_titled, width = 14, height = 12, dpi = 300)
```
...
df_pca.cwm.scores.traits$target_gene=as.factor(df_pca.cwm.scores.traits$target_gene)
...
model_initial <- mgcv::bam(
  Shape_boxcox ~ Comp.1 + Comp.2 +
    s(Biome, bs = "re") + s(target_gene, bs = "re") + 
...
plot(model.gam.shape, pages=1, all.terms=TRUE)
...
boxcox_model <- boxcox(lm(Volume ~ 1, data = df_pca.cwm.scores.traits))
