...
library(pacman)
pacman::p_load(readr, dplyr, tidyverse, factoextra, MASS, mgcv, ggplot2, patchwork, ggpubr, MASS, bestNormalize, ggmap, shadowtext)
```
#data upload
```{r}
...
length(unique(df_metadata.biogeography.traits.one.id.removed$id))#3,913
...
inset_plot <- ggplot(biome_counts, aes(x = Biome, y = n, fill = Biome)) +
  ...
  geom_shadowtext(aes(label = n), vjust = -0.5, size = 2.2, color = "black", bg.color = "white", bg.r = 0.15) +
  ylab("No. of Sites") +
