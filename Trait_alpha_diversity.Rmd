...
library(pacman)
pacman::p_load(readr, dplyr, tidyverse, factoextra, MASS, mgcv, ggplot2, patchwork, ggpubr, MASS, bestNormalize, fundiversity, Matrix)
```
...
str(df_metadata)
```
#Figure 3: Functiional diversity

```{r}

df_abundance.fundiversity <- df_metadata.biogeography.traits %>% filter(id!=1151)
colnames(df_abundance.fundiversity)

df_fundiversity <- na.omit(df_fundiveristy)
length(unique(df_fundiversity$id))#1,312
...
#sites and species
colnames(df_global.am_trait.am)
...
```{r}
df_fundiveristy.environment <- df_fundiversity.abundance.climate %>% dplyr::select(MAT, MAP,pH, Temp.annual.worldclim, Temp.seasonality.worldclim, Temp.max.worldclim, Temp.min.worldclim, Prec.annual.worldclim, Prec.seasonality.worldclim, Prec.max.worldclim, Prec.min.worldclim, mean_annual_vapor_worldclim, mean_annual_wind)
...
df_pca.fundiversity.scores.traits <- data.frame(pca.fundiversity$scores, Volume=df_fundiversity.abundance.climate$vol_mean, Ornamentation=df_fundiversity.abundance.climate$orn_height_mean, Investment=df_fundiversity.abundance.climate$investment_mean,  Shape=df_fundiversity.abundance.climate$shape_median, Color=df_fundiversity.abundance.climate$color_most, latitude=df_fundiversity.abundance.climate$Latitude_Decimal, longitude=df_fundiversity.abundance.climate$Longitude_Decimal, Biome=df_fundiversity.abundance.climate$Biome, sample_type=df_fundiversity.abundance.climate$sample_type, target_gene=df_fundiversity.abundance.climate$target_gene, sequencing_platform=df_fundiversity.abundance.climate$sequencing_platform, time=df_fundiversity.abundance.climate$year_of_sampling,
fun.richness= df_fundiversity.abundance.climate$FRic,
fun.evenness=df_fundiversity.abundance.climate$FEve,
fun.divergence=df_fundiversity.abundance.climate$FDiv)
```
...
# === 3. CALCULATE NULL SUMMARY STATISTICS ===
...
df_pca.fundiversity.scores.traits$target_gene=as.factor(df_pca.fundiversity.scores.traits$target_gene)
df_pca.fundiversity.scores.traits$sequencing_platform=as.factor(df_pca.fundiversity.scores.traits$sequencing_platform)
df_pca.fundiversity.scores.traits$time=as.numeric(df_pca.fundiversity.scores.traits$time)
hist(df_pca.fundiversity.scores.traits$fun.divergence)


###############################################functional richness

model.fun.rich = mgcv::bam(log(fun.richness)~ Comp.1 + Comp.2 +  s(Biome, bs = "re")  +s(target_gene, bs = "re") +s(sequencing_platform, bs = "re") + s(sample_type, bs = "re") + s(time, bs = "re")+ s(latitude, longitude, bs = "sos"),  method = "REML", data = df_pca.fundiversity.scores.traits)
...
fun.rich.pc1.fig <- ggplot() +
  ...
  geom_point(data = df_pca.fundiversity.scores.traits, 
             aes(x = Comp.1, y = log(fun.richness), fill = Biome), 
  ...
  geom_smooth(data = df_pca.fundiversity.scores.traits, 
              aes(x = Comp.1, y = log(fun.richness), fill = Biome, linetype = Biome), 
              method = "lm", se = FALSE, size = 3, color = "white", alpha = 0.5) +  # Glow layer
  geom_smooth(data = df_pca.fundiversity.scores.traits, 
              aes(x = Comp.1, y = log(fun.richness), color = Biome, linetype = Biome), 
...
fun.rich.pc2.fig <- ggplot() +
  # Add the observed data points
  geom_point(data = df_pca.fundiversity.scores.traits, 
             aes(x = Comp.2, y = log(fun.richness), fill = Biome), 
             size = 3, shape = 21, stroke = 0.5, color = "white", alpha = 0.8) +
  ...
  # Add glow effect for biome-specific lines
  geom_smooth(data = df_pca.fundiversity.scores.traits, 
              aes(x = Comp.2, y = log(fun.richness), fill = Biome, linetype = Biome), 
              method = "lm", se = FALSE, size = 3, color = "white", alpha = 0.5) +  # Glow layer
  geom_smooth(data = df_pca.fundiversity.scores.traits, 
              aes(x = Comp.2, y = log(fun.richness), color = Biome, linetype = Biome), 
              method = "lm", se = FALSE, size = 1.5) + scale_linetype_manual(values = c("dashed", "dashed", "dashed", "dashed", "dashed", "dashed", "dashed", "dashed")) +
  theme(legend.position = "right",  # Place the legend on the right
    legend.title = element_text(size = 14),  # Adjust legend title size
    legend.text = element_text(size = 14), # Italic subtitle
...
##########################functional evenness
model.fun.even = mgcv::bam(log(fun.evenness)~ Comp.1 + Comp.2 +  s(Biome, bs = "re")  +s(target_gene, bs = "re") +s(sequencing_platform, bs = "re") + s(sample_type, bs = "re") + s(time, bs = "re")+ s(latitude, longitude, bs = "sos"),  method = "REML", data = df_pca.fundiversity.scores.traits)
...
fun.even.pc1.fig <- ggplot() +
  # Add the observed data points
  geom_point(data = df_pca.fundiversity.scores.traits, 
             aes(x = Comp.1, y = log(fun.evenness), fill = Biome), 
             size = 3, shape = 21, stroke = 0.5, color = "white", alpha = 0.8) +
  ...
  # Add glow effect for biome-specific lines
  geom_smooth(data = df_pca.fundiversity.scores.traits, 
              aes(x = Comp.1, y = log(fun.evenness), fill = Biome, linetype = Biome), 
              method = "lm", se = FALSE, size = 3, color = "white", alpha = 0.5) +  # Glow layer
  geom_smooth(data = df_pca.fundiversity.scores.traits, 
              aes(x = Comp.1, y = log(fun.evenness), color = Biome, linetype = Biome), 
              method = "lm", se = FALSE, size = 1.5) + scale_linetype_manual(values = c("dashed", "dashed", "dashed", "dashed", "dashed", "dashed", "dashed", "dashed")) +
  theme(legend.position = "right",  # Place the legend on the right
    legend.title = element_text(size = 14),  # Adjust legend title size
    legend.text = element_text(size = 14), # Italic subtitle
...
fun.even.pc2.fig <- ggplot() +
  # Add the observed data points
  geom_point(data = df_pca.fundiversity.scores.traits, 
             aes(x = Comp.2, y = log(fun.evenness), fill = Biome), 
             size = 3, shape = 21, stroke = 0.5, color = "white", alpha = 0.8) +
  ...
  # Add glow effect for biome-specific lines
  geom_smooth(data = df_pca.fundiversity.scores.traits, 
              aes(x = Comp.2, y = log(fun.evenness), fill = Biome, linetype = Biome), 
              method = "lm", se = FALSE, size = 3, color = "white", alpha = 0.5) +  # Glow layer
  geom_smooth(data = df_pca.fundiversity.scores.traits, 
              aes(x = Comp.2, y = log(fun.evenness), color = Biome, linetype = Biome), 
              method = "lm", se = FALSE, size = 1.5) + scale_linetype_manual(values = c("dashed", "dashed", "dashed", "dashed", "dashed", "dashed", "dashed", "dashed")) +
  theme(legend.position = "right",  # Place the legend on the right
    legend.title = element_text(size = 14),  # Adjust legend title size
    legend.text = element_text(size = 14), # Italic subtitle
...
model.fun.div = mgcv::bam(log(fun.divergence)~ Comp.1 + Comp.2 +  s(Biome, bs = "re")  +s(target_gene, bs = "re") +s(sequencing_platform, bs = "re") + s(sample_type, bs = "re") + s(time, bs = "re")+ s(latitude, longitude, bs = "sos"),  method = "REML", data = df_pca.fundiversity.scores.traits)
...
fun.div.pc1.fig <- ggplot() +
  # Add the observed data points
  geom_point(data = df_pca.fundiversity.scores.traits, 
             aes(x = Comp.1, y = log(fun.divergence), fill = Biome), 
             size = 3, shape = 21, stroke = 0.5, color = "white", alpha = 0.8) +
  ...
  # Add glow effect for biome-specific lines
  geom_smooth(data = df_pca.fundiversity.scores.traits, 
              aes(x = Comp.1, y = log(fun.divergence), fill = Biome, linetype = Biome), 
              method = "lm", se = FALSE, size = 3, color = "white", alpha = 0.5) +  # Glow layer
  geom_smooth(data = df_pca.fundiversity.scores.traits, 
              aes(x = Comp.1, y = log(fun.divergence), color = Biome, linetype = Biome), 
              method = "lm", se = FALSE, size = 1.5) + scale_linetype_manual(values = c("dashed", "dashed", "dashed", "dashed", "dashed", "dashed", "dashed", "dashed")) +
  theme(legend.position = "right",  # Place the legend on the right
    legend.title = element_text(size = 14),  # Adjust legend title size
    legend.text = element_text(size = 14), # Italic subtitle
...
fun.div.pc2.fig <- ggplot() +
  # Add the observed data points
  geom_point(data = df_pca.fundiversity.scores.traits, 
             aes(x = Comp.2, y = log(fun.divergence), fill = Biome), 
             size = 3, shape = 21, stroke = 0.5, color = "white", alpha = 0.8) +
  ...
  # Add glow effect for biome-specific lines
  geom_smooth(data = df_pca.fundiversity.scores.traits, 
              aes(x = Comp.2, y = log(fun.divergence), fill = Biome, linetype = Biome), 
              method = "lm", se = FALSE, size = 3, color = "white", alpha = 0.5) +  # Glow layer
  geom_smooth(data = df_pca.fundiversity.scores.traits, 
              aes(x = Comp.2, y = log(fun.divergence), color = Biome, linetype = Biome), 
              method = "lm", se = FALSE, size = 1.5) + scale_linetype_manual(values = c("dashed", "dashed", "dashed", "dashed", "dashed", "dashed", "dashed", "dashed")) +
  theme(legend.position = "right",  # Place the legend on the right
    legend.title = element_text(size = 14),  # Adjust legend title size
    legend.text = element_text(size = 14), # Italic subtitle
